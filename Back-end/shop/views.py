...
class ProductViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if sortBy in ["price","rating"]:
            order = "" if sortDir == "asc" else "-"; qs = qs.order_by(f"{order}{sortBy}", "id")
        else:
            qs = qs.order_by("-id")
        return qs
...
class OrderViewSet(mixins.CreateModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        addr = data["address"]; items = data["items"]
        if not items:
        ...
        for it in items:
            try:
                ...
                order.delete(); return Response({"detail": f"Invalid product {it['product_id']}"}, status=400)
            qty = it["qty"]
            price_cents = int(Decimal(p.price) * 100)
            OrderItem.objects.create(order=order, product=p, qty=qty, price_cents=price_cents)
            total += qty * price_cents
        order.total_cents = total; order.save()
        return Response(OrderSerializer(order).data, status=status.HTTP_201_CREATED)

@api_view(["POST"])
@permission_classes([IsAuthenticated])
def create_intent(request):
    amount = int(request.data.get("amount_cents", 0)); currency = request.data.get("currency","usd")
    if amount <= 0: return Response({"detail":"amount_cents must be > 0"}, status=400)
